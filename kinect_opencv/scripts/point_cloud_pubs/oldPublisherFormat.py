...
def process_camera(image, camera_info, depth):
    ...
    try:
        ...
        listener.waitForTransform("/odom","/temperature_back_link", rospy.Time(0), rospy.Duration(3.0))
        (trans,rot) = listener.lookupTransform("/odom","/temperature_back_link", rospy.Time(0))
        ...
        if len(red_pixels_x) > 0:
            ...
            for i in range(len(pos_world_frame)):
                pointCloudOut.points.append(Point32(pos_world_frame[i][0], pos_world_frame[i][1], pos_world_frame[i][2])) 
        else:
            #Let's user know when no red pixels have been detected
            for i in range(len(pos_world_frame)):
                pointCloudOut.points.append(Point32(pos_world_frame[0], pos_world_frame[1], pos_world_frame[2]))
...
if __name__ == '__main__':
    try:
        pointcloud_publisher = rospy.Publisher("/temperature_back_pointcloud_topic", PointCloud, queue_size = 10)
        start_node()
