...
def start_node():
    ...
    rospy.init_node('temperature_right_processor')
    rospy.loginfo('temperature_right_processor node started')
    
    #Subscribe to the camera, point cloud and camera info topics
    image_sub = message_filters.Subscriber("/temperature_right/color/image_raw", Image)
    info_sub = message_filters.Subscriber("/temperature_right/color/camera_info", CameraInfo)
    depth_sub = message_filters.Subscriber("temperature_right/depth/points", PointCloud2)
...
if __name__ == '__main__':
    try:
        pointcloud_publisher = rospy.Publisher("/temperature_right_pointcloud_topic", StampedArray, queue_size = 10)
        start_node()
