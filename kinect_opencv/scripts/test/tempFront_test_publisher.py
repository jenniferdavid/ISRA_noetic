...
def process_camera(image, camera_info, depth):
    ...
    try:
        ...
        listener.waitForTransform("/odom","/temperature_front_link", rospy.Time(0), rospy.Duration(3.0))
        (trans,rot) = listener.lookupTransform("/odom","/temperature_front_link", rospy.Time(0))
        ...
        #Determines where the mask is equal to 0  (i.e. where the blue pixels are)
        ...
        if len(blue_pixels_x) > 0:
            ...
            point_cloud.append(pos_world_frame[0])
            point_cloud.append(pos_world_frame[1])
            point_cloud.append(pos_world_frame[2])
...
def start_node():
    ...
    rospy.init_node('temperature_front_processor')
    rospy.loginfo('temperature_front_processor node started')
    
    #Subscribe to the camera, point cloud and camera info topics
    image_sub = message_filters.Subscriber("/temperature_front/color/image_raw", Image)
    info_sub = message_filters.Subscriber("/temperature_front/color/camera_info", CameraInfo)
    depth_sub = message_filters.Subscriber("temperature_front/depth/points", PointCloud2)
...
if __name__ == '__main__':
    try:
        pointcloud_publisher = rospy.Publisher("/temperature_front_pointcloud_topic", StampedArray, queue_size = 10)
        start_node()
