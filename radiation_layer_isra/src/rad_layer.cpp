#include<radiation_layer_isra/rad_layer.h>
...
PLUGINLIB_EXPORT_CLASS(radiation_layer_isra_namespace::RadLayer, costmap_2d::Layer)
...
namespace radiation_layer_isra_namespace
{
...
void RadLayer::onInitialize()
{
  ...
  node_name_ = ros::this_node::getName() + "/" + name_; // potato - Useful for logging to user console when many layers are running
  current_ = true;
  ...
  use_lethal_ = false; //potato
  max_cost_ = INSCRIBED_INFLATED_OBSTACLE - 1; // potato


  // Add dynamic reconfigure to layer
  dsrv_ = new dynamic_reconfigure::Server<radiation_layer_isra::RadiationLayerConfig>(nh);
  dynamic_reconfigure::Server<radiation_layer_isra::RadiationLayerConfig>::CallbackType cb = boost::bind(
...
void RadLayer::reconfigureCB(radiation_layer_isra::RadiationLayerConfig &config, uint32_t level)
{
  ...
  //if (config.lower_threshold != lower_threshold_ || config.lower_threshold_scale != lower_threshold_scale_ || config.upper_threshold != upper_threshold_ || config.upper_threshold_scale != upper_threshold_scale_) {
    ...
    bool previous_enabled = enabled_; //potato
    ...
    //potato - everything below here is new
...
unsigned int RadLayer::scaledValue(float value)
{
  ...
  // potato - between potatos is new, above is old code commented
