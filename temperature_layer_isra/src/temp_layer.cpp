#include<temperature_layer_isra/temp_layer.h>
...
PLUGINLIB_EXPORT_CLASS(temperature_layer_isra_namespace::TempLayer, costmap_2d::Layer)
...
void TempLayer::onInitialize()
{
  ros::NodeHandle nh("~/" + name_), g_nh;
  node_name_ = ros::this_node::getName() + "/" + name_; // Useful for logging to user console when many layers are running potato
  current_ = true;
  ...
  // Get temperature topic name
  std::string temperature_topic;
  nh.param("temperature_topic", temperature_topic, std::string("temperature_topic"));

  inflate_temperature_ = false;
  ...
  // Setup subscriber to topic, callback function = temperatureCB
  temperature_sub_ = g_nh.subscribe(temperature_topic, 1, &TempLayer::temperatureCB, this);
  ...
  dsrv_ = new dynamic_reconfigure::Server<temperature_layer_isra::TemperatureLayerConfig>(nh);
  dynamic_reconfigure::Server<temperature_layer_isra::TemperatureLayerConfig>::CallbackType cb = boost::bind(
      &TempLayer::reconfigureCB, this, _1, _2);
  dsrv_->setCallback(cb);
}


void TempLayer::matchSize()
{
  ...
  if (averages_ == NULL) {
    //ROS_WARN("TempLayer::updateCosts(): averages_ array is NULL");
  ...
  {
    //ROS_WARN("TempLayer::updateCosts(): averages_ array size is wrong");
    delete[] averages_;
    averages_size_ = size_x * size_y;
    averages_ = new float[averages_size_];
    weight_obs_ = new float[averages_size_];
...
void TempLayer::reconfigureCB(temperature_layer_isra::TemperatureLayerConfig &config, uint32_t level)
{
...
void TempLayer::temperatureCB(const sensor_msgs::Image& temp_msg)
{
  boost::recursive_mutex::scoped_lock lock(lock_);  // Ensure only this functon can interact with temperature temperature_msg_buffer_
  temperature_msg_buffer_.push_back(temp_msg);  // Place new message onto buffer
  boost::recursive_mutex::scoped_lock unlock(lock_);  // Release control of the buffer
}

void TempLayer::updateObservations(std::list<std::pair<unsigned int, float> > &updates)
{
  std::list<sensor_msgs::Image> temperature_msg_buffer_copy_;  // Create blank buffer for observations

  boost::recursive_mutex::scoped_lock lock(lock_);  // Restrict access to buffer to this function only (i.e. can't be updated via temperatureCB())
  temperature_msg_buffer_copy_ = std::list<sensor_msgs::Image>(temperature_msg_buffer_);  // Make copy of current observation buffer
  temperature_msg_buffer_.clear();  // Clear current observation buffer for new messages
  ...
  if (temperature_msg_buffer_copy_.size() == 0) {   //  If no observations are available for processing then return
    return;
  }

  // Loop over all observations
  for (std::list<sensor_msgs::Image>::iterator obs = temperature_msg_buffer_copy_.begin(); obs != temperature_msg_buffer_copy_.end(); obs++) {
    ...
    try
    {
      ...
      ///////////// *** Changed the time at which the frames are passed to remove issues with timing *** //////////////
    ...
    // Extract temperature data value from image message - associated with camera green channel
    float value = obs -> data[0];  // Retrive 2nd channel (index 1) from data field in message
    ...
    } else {  // end if inflate_temperature_
      costmap_2d::MapLocation loc;
...
void TempLayer::getCache(std::list<std::pair<std::pair<double,double>, std::pair<float,float> > > &observation_cache)
{
  float* averages_copy;
...
unsigned int TempLayer::scaledValue(float value)
{
  ...
  // }
  //potato - between potatoes is the potato
...
void TempLayer::updateBounds(double robot_x, double robot_y, double robot_yaw, double* min_x,
                                           double* min_y, double* max_x, double* max_y)
{
  // If layer not enabled, do not record data to local costmap - not sure why you would _want_ to do this for temperature
  // if (!enabled_)
  //   return;

  if (rolling_window_){
    matchSize(); // Will cache all current temperature data, plus set the origin etc of the map
  }
...
void TempLayer::updateCosts(costmap_2d::Costmap2D& master_grid, int min_i, int min_j, int max_i,
                                          int max_j)
{
  if (!enabled_){
    return;
...
void TempLayer::updatePreserveNoInfo(costmap_2d::Costmap2D& master_grid, int min_i, int min_j, int max_i, int max_j)
{
  if (!enabled_)
    return;
...
std::vector<geometry_msgs::Point> TempLayer::makeSensorFootprintFromParams(ros::NodeHandle& nh)
{
  std::string full_param_name;
  std::string full_radius_param_name;
  std::vector<geometry_msgs::Point> points;

  if (nh.searchParam("temperature_footprint", full_param_name))
  {
  ...
  if (nh.searchParam("temperature_radius", full_radius_param_name))
  {
    double temperature_radius;
    nh.param(full_radius_param_name, temperature_radius, 0.0);
    points = costmap_2d::makeFootprintFromRadius(temperature_radius);
    nh.setParam("temperature_radius", temperature_radius);
    inflate_temperature_ = true;
  }
  return points;
}
